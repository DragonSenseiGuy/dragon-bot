...
def cap_timeout_duration(duration: Union[datetime.datetime, relativedelta]) -> tuple[bool, datetime.datetime]:
...
async def notify_timeout_cap(bot: discord.Client, ctx: Union[Context, discord.Interaction], user: discord.Member) -> None:
    ...
    if is_mod_channel(ctx.channel):
        ...
        if mod_channel:
            await mod_channel.send(
                f":warning: {author.mention} {cap_message_for_user}")