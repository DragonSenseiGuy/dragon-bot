    ...
    async def apply_timeout(self, ctx, user, reason, duration_or_expiry):
        ...
        if ctx.guild:
            ...
            if user.top_role >= me.top_role:
                await reply(":x: I cannot timeout this user because their role is higher than or equal to mine.", ephemeral=True)
                return

        try:
            await user.timeout(duration_or_expiry, reason=reason)
        except discord.Forbidden:
            await reply(":x: I don't have permission to timeout this user.", ephemeral=True)
            return
        except discord.HTTPException as e:
        ...
        if duration_or_expiry:
             await reply(f":white_check_mark: Timed out {user.mention} for {duration_or_expiry}.", ephemeral=True)
        else:
    ...
    async def timeout(
        ...
        *,
        ...
        if not isinstance(user, Member):
            await interaction.response.send_message(":x: The user doesn't appear to be on the server.", ephemeral=True)
            return
                ...
                if delta:
                    ...
                    try:
                        duration_obj = now + delta
                         ...
                         await interaction.response.send_message(f"`{duration}` results in a datetime outside the supported range.", ephemeral=True)
                         return
                else:
                     await interaction.response.send_message(f"`{duration}` is not a valid duration string or ISO-8601 datetime.", ephemeral=True)
                     return
